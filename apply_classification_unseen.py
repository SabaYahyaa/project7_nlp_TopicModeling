...
def apply_model_xgboost(lst_text):
    map_dic_topics = {
    ...
    for t in lst_text:
        cleaned_text=None
        try:
            cleaned_text = CleanText(t) #create a list of token
        except:
            cleaned_text=None
        all_data.append(cleaned_text)

    #get the tfidf vectorizer, to apply it on unseen data
    vectorizer = joblib.load('{}.pkl'.format('./data/tf_vect'))
    vect_train = vectorizer.transform(all_data)  #on unseen data, vect

    #use the saved X_gboost to get prediction
    ##### save the model, although we have overfit (please, use GridSearch to find the optimal parameters
    Xgboost=joblib.load('./data/model_xgboost.pkl')
    dominated_topics=Xgboost.predict(vect_train)
...
if __name__=="__main__":
    ...
    topics=apply_model_xgboost(lst_text)
    print(topics)
